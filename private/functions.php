...
function minutes_menu($value, $select_name) {
  echo '<select name="'.$select_name.'" id="data">';
  for ($i = 0; $i <= 59; $i++) {
    $i_padded = sprintf("%02d", $i);
    echo ($i_padded == $value) ? "<option id='font' selected>" : "<option id='font'>";
    echo $i_padded;
    echo "</option>";
  }
  echo '</select>';
...
function convert_seconds($secs) {
	...
	//seconds
	$s = $secs % 60;
	$output = sprintf("%02d:%02d:%02d", $H, $i, $s);
	return $output;
}
...
function find_proposta_by_id($id) {
	global $handle;
	$safe_id = mysqli_real_escape_string($handle, $id);
   	$query = "SELECT * FROM proposte WHERE id = {$safe_id} LIMIT 1;";
    $proposta = mysqli_query($handle, $query);
	confirm_query($proposta);
	return $proposta;
}
...
function verify_status_proposte($user) {
	global $handle;
	$buyer = mysqli_real_escape_string($handle, $user);
	$query1= "SELECT * FROM proposte WHERE buyer = '$buyer' AND stato = 'in attesa HQ' LIMIT 1;";
	$result = mysqli_query($handle, $query1);
	//confirm_query($result);
	if ($array = mysqli_fetch_assoc($result)) { return $array; }
	else {
		$query2= "SELECT * FROM proposte WHERE buyer = '$buyer' AND stato = 'in attesa buyer' LIMIT 1;";
		$result = mysqli_query($handle, $query2);
		//confirm_query($result);
		if ($array = mysqli_fetch_assoc($result)) { return $array; }
		else { return null; }
	}
